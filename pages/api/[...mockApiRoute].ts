...
import { UnparsedClearSkyData, UnparsedForecastData } from '~/lib/types';
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method == 'GET') {
    if (
      ...
      const pvActualJsonFake = fakeDates(
        pvActualJson as any,
        'forecast_values'
      );
      ...
      const pvForecastJSONFake = fakeDates(
        pvForecastJson as any,
        'forecast_values'
      );
      ...
      const clearskyJsonFake = fakeDates(
        clearskyJson as any,
        'clearsky_estimate'
      );
      res.status(200).json(clearskyJsonFake);
    } else if (
      mockApiRoute ===
...
function fakeDates(forecastData: any, valuesKey: any) {
  const today = new Date();
  const oldStart = new Date(
    parseNowcastingDatetime(forecastData[valuesKey][0].target_datetime_utc)
  );
  const newStart = new Date(oldStart);
  newStart.setUTCDate(today.getDate());
  const difference = newStart.getTime() - oldStart.getTime();
  const forecast_values = forecastData[valuesKey].map((value: any) => {
