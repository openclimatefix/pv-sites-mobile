...
import { ClearSkyDataPoint, ForecastDataPoint, SiteList } from './types';
import { getCurrentTimeForecastIndex } from './graphs';

export const getTotalExpectedForecastOutput = (points: ForecastDataPoint[]) => {
  let approxArea = 0;
...
export const getTotalExpectedClearSkyOutput = (points: ClearSkyDataPoint[]) => {
  let approxArea = 0;
  const millisInHour = 3.6e6;

  for (let i = 0; i < points.length - 1; i++) {
    const avgHeight =
      (points[i].clearsky_generation_kw +
        points[i + 1].clearsky_generation_kw) /
      2;
    const timeDiffMilli =
      new Date(points[i + 1].target_datetime_utc).getTime() -
      new Date(points[i].target_datetime_utc).getTime();
